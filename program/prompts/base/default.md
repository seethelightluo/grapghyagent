...
- **TaskCreate** / **TaskUpdate** / **TaskGet** / **TaskList** / **TaskGateCheck**: structured task list with `blocks` / `blocked_by` edges, I/O contracts, gate conditions, and audit fields.
- **TaskRetry** / **TaskDecompose** / **TaskExecuteRecovery**: built-in recovery for failed tasks. `TaskExecuteRecovery` runs the full pipeline (attempt → retry with failure context → decompose into sub-tasks). Max decomposition depth is configurable (default 2).

**Workflow:** break multi-step plans into tasks at the start → define I/O contracts and examples → set gate conditions → mark in_progress when starting each → use `TaskExecuteRecovery` for automatic retry/decompose on failure → verify outputs → open/close gates → mark completed when done → use TaskList to review.

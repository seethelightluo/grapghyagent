...
def cmd_tasks(args: str, _state, config) -> bool:
    ...
    if subcmd == "get":
        ...
        if t.input_spec:    print(f"  Input spec:  {t.input_spec}")
        if t.output_spec:   print(f"  Output spec: {t.output_spec}")
        if t.audit_status:  print(f"  Audit:       {t.audit_status}")
        if t.necessity_claim: print(f"  Necessity:   {t.necessity_claim}")
        if t.verification_rule: print(f"  Verify:      {t.verification_rule}")
        if t.acceptance_status: print(f"  Acceptance:  {t.acceptance_status}")
        if t.compressed_judgment: print(f"  Judgment:    {t.compressed_judgment}")
        if t.evidence_pointer: print(f"  Evidence:    {t.evidence_pointer}")

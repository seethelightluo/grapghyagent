...
_BUILTIN_AGENTS: Dict[str, AgentDefinition] = {
    ...
    "auditor": AgentDefinition(
        name="auditor",
        description="Independent audit agent that verifies task graph nodes, evidence pointers, and necessity claims.",
        system_prompt=(
            "You are an independent auditor for verifiable task graphs.\n"
            "Your job is to verify, NOT to execute.\n\n"
            "For each node you audit:\n"
            "1. Check verification_result: are the specific claims backed by evidence?\n"
            "2. Resolve evidence_pointers: do they point to real, valid data?\n"
            "3. Validate necessity_audit: does the counterfactual hold? Would removing the node break the goal?\n"
            "4. Compare input_example/output_example with actual_input/actual_output:\n"
            "   - Are they structurally consistent?\n"
            "   - Do actuals meet the contract?\n"
            "5. Verify gate conditions: should this node's gate be open or closed?\n\n"
            "Output a structured audit report:\n"
            "- PASS/FAIL per node\n"
            "- Specific discrepancies found\n"
            "- Confidence level (0.0-1.0)\n"
            "- Recommendations\n\n"
            "Be skeptical. Your job is to catch what the executor missed.\n"
        ),
        tools=["Read", "Glob", "Grep", "TaskGet", "TaskList", "TaskGateCheck"],
        source="built-in",
    ),
}
...
class SubAgentManager:
    ...
    def spawn(
        ...
        if agent_def:
            ...
            if agent_def.tools:
                eff_config["_allowed_tools"] = agent_def.tools
